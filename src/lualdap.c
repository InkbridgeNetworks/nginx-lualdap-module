...
proxy_control_add(conn_data *conn, const struct berval *authz_id)
{
	...
	if (conn->proxy_authz_id && conn->proxy_authz_id_len == authz_id->bv_len && memcmp(conn->proxy_authz_id, authz_id->bv_val, authz_id->bv_len) == 0)
...
static int lualdap_close(lua_State *L)
{
	...
	if (!was_open)
		return 0; /* preserve old "already closed -> nothing" return */
	lua_pushnumber(L, 1);
	return 1;
}
...
static void push_entry(lua_State *L, LDAP *ld, LDAPMessage *entry)
{
	...
	(void)ld;
	if ((ber == NULL) || (ber_scanf(ber, "{" /*}*/) == LBER_ERROR) || (ber_get_stringbv(ber, &dn, LBER_BV_NOTERM) == LBER_DEFAULT)) {
		lua_pushnil(L);
		lua_newtable(L);
		if (ber)
			ber_free(ber, 0);
	...
	for (tag = ber_first_element(ber, &len, &attr_last); tag != LBER_DEFAULT;
		...
		if (ber_scanf(ber, "{" /*}*/) == LBER_ERROR)
			break;
		if (ber_get_stringbv(ber, &name, LBER_BV_NOTERM) == LBER_DEFAULT)
			break;
		...
		for (tag = ber_first_element(ber, &len, &val_last); tag != LBER_DEFAULT;
			...
			if (ber_get_stringbv(ber, &val, LBER_BV_NOTERM) == LBER_DEFAULT)
				break;
			n++;
			if (n == 2) {
...
static int lualdap_search_persistent(lua_State *L)
{
	...
	if (ber == NULL)
		return luaL_error(L, LUALDAP_PREFIX "Failed allocating ber for sync control");
	...
	*ctrls_p = NULL;
	...
	/*
	 * derefAliases is forced to neverDerefAliases at connection setup
	 * (init_fd), so no per-search override is needed - syncprov's
	 * 'illegal value for derefAliases' protocol error can't fire here.
	 */
...

	{
		...
		if (cookie)
			s->latest_cookie = ber_bvdup(cookie);
	}
	lua_pushvalue(L, -1);
